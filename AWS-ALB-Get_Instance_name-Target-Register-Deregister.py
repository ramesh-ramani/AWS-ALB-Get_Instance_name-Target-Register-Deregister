...
##The Below lines will deregister the targets from a target group##

    response = elbList.deregister_targets(
        TargetGroupArn=k,
        Targets=[
            {
                'Id': v,
...
##Below lines will check if you want to register the deregistered target back again, and if yes, then put the target back into the Target Group##


    while True:
          h=raw_input("Do you want to Register the instance back to the Target Group (Y/N)? : ")
          if h=='Y': break
